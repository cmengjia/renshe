<template>
	<div class="contacts">
		<div class="search" v-show="showSearch">
			<span class="checkbox" :class="{checked:isCheckAll}" @click.stop="onCheckAll" v-if="hasCheck&&!isSingle" v-show="!isSearch">
				<label></label>
			</span>
		...
		<div class="list" :style="calcHeight" v-if="!isSearch">
	    	<ul>
    	...
    	<div class="selected" v-if="showSelectedUser.length>0">
    		<ul>
    			<li v-for="item in showSelectedUser" @click="onRemove(item)">
    				<img :id="item.id" :src="item.photo" width="50" height="50">
	    		</li>
    		</ul>
    		<slot name="button">
...
export default { 
    ...
    computed:{
    	calcHeight:function(){//计算list的高度
    		let height = document.documentElement.clientHeight;
    		if(this.showSelectedUser.length>0){
    			height = height - 70 - 75;
    		}else{
    			height = height - 70;
    		}
